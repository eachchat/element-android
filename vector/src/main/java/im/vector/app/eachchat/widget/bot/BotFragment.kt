...
class BotFragment @Inject constructor(
) : VectorBaseFragment<FragmentMyContactsBinding>() {
    private val adapter = BotAdapter()
    private var botLiveData: LiveData<List<Bot>?> =
            AppDatabase.getInstance(BaseModule.getContext()).botDao().getBotsLive()

    private val viewModel: BotViewModel by fragmentViewModel()
...
//        loadingText = getString(R.string.loading_my_contacts)
        ...
        botLiveData.observe(viewLifecycleOwner) {
            viewModel.loadBot(it)
        }

        viewModel.loading.observe(viewLifecycleOwner) {
            if (it) {
                showLoading(getString(R.string.loading_bot))
            } else {
...
//                startActivity(intent)
//            }
            BotInfoActivity.start(requireContext(), BotInfoArg(userId = user.matrixId, displayName = user.displayName))
        }
    }
