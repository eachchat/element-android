...
class MyContactViewModel @AssistedInject constructor(
    ...
    @AssistedFactory
    interface Factory : MavericksAssistedViewModelFactory<MyContactViewModel, EmptyViewState> {
        override fun create(initialState: EmptyViewState): MyContactViewModel
    }

    companion object : MavericksViewModelFactory<MyContactViewModel, EmptyViewState> by hiltMavericksViewModelFactory() {
    ...
    fun loadMyContacts(contacts: List<ContactsDisplayBeanV2?>?) {
        if (contacts == null) {
            return
        }
        viewModelScope.launch(Dispatchers.IO) {
