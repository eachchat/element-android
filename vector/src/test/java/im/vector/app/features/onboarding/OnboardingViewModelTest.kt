...
class OnboardingViewModelTest {
    ...
    @Test
    fun `when handling PersonalizeProfile action then emits OnPersonalizeProfile event`() = runBlockingTest {
        val test = viewModel.test(this)

        viewModel.handle(OnboardingAction.PersonalizeProfile)

        test
                .assertEvents(OnboardingViewEvents.OnPersonalizeProfile)
                .finish()
    }

    @Test
    fun `when handling TakeMeHome action then emits OnTakeMeHome event`() = runBlockingTest {
        val test = viewModel.test(this)
